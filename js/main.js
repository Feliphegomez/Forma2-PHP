	...
	Forma2.app("POST", "quiz", 
	{
		...
		if(r.error === false){
			if(r.quiz.presencia == false && r.quiz.querys.length > 0){
				...
				$link = Forma2.options.site_url+'index.php?pageActive=f5-quiz&piloto='+Forma2.loadSession().authResponse.signedRequest.piloto+'&id='+Forma2.loadSession().authResponse.signedRequest.id;
				...
				if(Forma2.loadSession().authResponse.signedRequest.permisos.quiz.edit == true){
...
function submitQuizCurrent(){
	...
	if(response.length > 0){
		...
		Forma2.app("POST", "quiz", 
		{
			...
			if(r.error === false){
				$.notify(r.message, "success");
				location.replace(Forma2.options.site_url+'index.php?pageActive=f5-result-last');
			}else{
				$.notify(r.message, "error");
				cargarQuizCurrent();
			}
...
function cargarBtnFloat(){
	...
	if(
		Forma2.loadSession().authResponse.signedRequest.permisos.ecards.create && Forma2.loadSession().authResponse.signedRequest.permisos.ecards.create == true
		|| Forma2.loadSession().authResponse.signedRequest.permisos.ecards.edit && Forma2.loadSession().authResponse.signedRequest.permisos.ecards.edit == true
		|| Forma2.loadSession().authResponse.signedRequest.permisos.articles.create && Forma2.loadSession().authResponse.signedRequest.permisos.articles.create == true
		|| Forma2.loadSession().authResponse.signedRequest.permisos.articles.edit && Forma2.loadSession().authResponse.signedRequest.permisos.articles.edit == true
		|| Forma2.loadSession().authResponse.signedRequest.permisos.alerts.create && Forma2.loadSession().authResponse.signedRequest.permisos.alerts.create == true
		|| Forma2.loadSession().authResponse.signedRequest.permisos.quiz.create && Forma2.loadSession().authResponse.signedRequest.permisos.quiz.create == true
	){
		$e = '';
		if(Forma2.loadSession().authResponse.signedRequest.permisos.ecards.create && Forma2.loadSession().authResponse.signedRequest.permisos.ecards.create == true){
			$e += crearItemBtnFloat({
		...
		if(Forma2.loadSession().authResponse.signedRequest.permisos.articles.create && Forma2.loadSession().authResponse.signedRequest.permisos.articles.create == true){
			$e += crearItemBtnFloat({
		...
		if(Forma2.loadSession().authResponse.signedRequest.permisos.alerts.create && Forma2.loadSession().authResponse.signedRequest.permisos.alerts.create == true){
			$e += crearItemBtnFloat({
		...
		if(Forma2.loadSession().authResponse.signedRequest.permisos.ecards.edit && Forma2.loadSession().authResponse.signedRequest.permisos.ecards.edit == true){
			$e += crearItemBtnFloat({
				"href":Forma2.options.site_url+"index.php?pageActive=saves-publishs",
				"icon":"fas fa-save",
				"title":"Borradores [Publicaciones]"
			});
		};
		if(Forma2.loadSession().authResponse.signedRequest.permisos.quiz.create && Forma2.loadSession().authResponse.signedRequest.permisos.quiz.create == true){
			$e += crearItemBtnFloat({
		...
		if(Forma2.loadSession().authResponse.signedRequest.permisos.quiz.edit && Forma2.loadSession().authResponse.signedRequest.permisos.quiz.edit == true){
			$e += crearItemBtnFloat({
				"href":Forma2.options.site_url+"index.php?pageActive=saves-quiz",
				"icon":"fas fa-save",
				"title":"Quiz Borradores"
			});
		};
		if(Forma2.loadSession().authResponse.signedRequest.permisos.quiz.delete && Forma2.loadSession().authResponse.signedRequest.permisos.quiz.delete == true){
			$e += crearItemBtnFloat({
...
function createPublishBorrador(type){
	...
	Forma2.app("POST", "publicaciones", 
	{
		...
		if(r.error === false){
			$.notify(r.message, "success");
			window.location.href = Forma2.options.site_url+'index.php?pageActive=single&type='+type+'&id_ref='+r.id;
		}else{
			$.notify(r.message, "error");
		}
	});
...
function createFilaTablaDraftsQuiz(element){
	...
	$ht = '<tr id="drafts-quiz-id-'+element.id+'">';
		...
		$ht += '<td><a href="'+Forma2.options.site_url+'index.php?pageActive=create-quiz&type=quiz&draft='+element.id+'"><i class="fas fa-toggle-on"></i></a></td>';
	$ht += '</tr>';
	return $ht;
}
...
function createFilaTablaDraftsPublish(element){
	...
	$ht = '<tr id="drafts-publish-id-'+element.id+'">';
		...
		$ht += '<td><a href="'+Forma2.options.site_url+'index.php?pageActive=single&type='+element.type+'&id_ref='+element.id+'">'+element.title+'</a></td>';
		$ht += '<td>'+element.category_name+'</td>';
		$ht += '<td>'+element.author.user+'</td>';
		$ht += '<td>'+element.fcreate+'</td>';
		$ht += '<td>'+element.fchange+'</td>';
		$ht += '<td><a href="'+Forma2.options.site_url+'index.php?pageActive=single&type='+element.type+'&id_ref='+element.id+'"><i class="fas fa-toggle-on"></i></a></td>';
	$ht += '</tr>';
	return $ht;
}
...
function createFilaTablaHistoryQuiz(element){
	...
	$ht = '<tr id="history-publish-id-'+element.id+'">';
		$ht += '<td><a href="'+Forma2.options.site_url+'index.php?pageActive=create-quiz&type=quiz&draft='+element.id+'">'+element.id+'</a></td>';
		$ht += '<td>'+element.title+'</td>';
		$ht += '<td>'+element.fecha_creation+'</td>';
		$ht += '<td><a href="'+Forma2.options.site_url+'index.php?pageActive=export-quiz&topic='+element.id+'"><i class="fas fa-toggle-on"></i></a></td>';
	$ht += '</tr>';
	return $ht;
}
				...
				$ha += '<div class="col-sm-12">';
					$ha += '<div class="testimonial testimonial-default-filled">';
						...
						$ha += '<div class="testimonial-desc">';
							$ha += '<img src="'+Forma2.loadSession().authResponse.signedRequest.avatar_url+'" alt="" />';
							$ha += '<div class="testimonial-writer">';
								$ha += '<div class="testimonial-writer-name">'+Forma2.loadSession().authResponse.signedRequest.nombre+'</div>';
		...
		$h += '<div id="questions-id-'+element.id+'" class="testimonial testimonial-'+$color+'-filled">';
			...
			$h += '<div class="testimonial-desc">';
				...
				$h += '<div class="testimonial-writer">';
					...
					if(Forma2.loadSession().authResponse.signedRequest.permisos.comments.response == true){
					...
					if(Forma2.loadSession().authResponse.signedRequest.permisos.comments.delete == true){
		...
		if (typeof element.tree !== 'undefined' && element.tree !== null) {
			...
			$h += '<div class="col-sm-12">';
				$h += '<div class="testimonial testimonial-default-filled">';
					...
					$h += '</div>';
					$h += '<div class="testimonial-desc">';
						$h += '<img src="'+Forma2.loadSession().authResponse.signedRequest.avatar_url+'" alt="" />';
						$h += '<div class="testimonial-writer">';
							$h += '<div class="testimonial-writer-name">'+Forma2.loadSession().authResponse.signedRequest.nombre+'</div>';
...
function cargarNavBarTop(){
	...
	if(Forma2.loadSession().authResponse.signedRequest.permisos.forum !== undefined && Forma2.loadSession().authResponse.signedRequest.permisos.forum.response == true){
		...
		//Forma2.options.api_url_large/comments.php?accesstoken= { ACCESSTOKEN } &action=pending&type=forum
	...
	if(Forma2.loadSession().authResponse.signedRequest.permisos.indicators !== undefined && Forma2.loadSession().authResponse.signedRequest.permisos.indicators.view == true){
		...
		//Forma2.options.api_url_large/kpis.php?accesstoken= { ACCESSTOKEN } &action=my_indicators
		console.log("Cargando Indicadores Activos");
		Forma2.app("POST", "kpis", 
		{
			"action":"view",
			"my":true
		}, function(r){
	...
	if(Forma2.loadSession().authResponse.signedRequest.permisos.alerts !== undefined && Forma2.loadSession().authResponse.signedRequest.permisos.alerts.view == true){
		...
		Forma2.app("POST", "alerts", 
		{
			...
			if(r.error === false){
				...
				if(
					Forma2.loadSession().authResponse.signedRequest.permisos && Forma2.loadSession().authResponse.signedRequest.permisos.alerts.create == true
				...
				for (i = 0; i < r.data.length; i++) {
					...
					if(
						...
						$html_a += crearItemAlertsSideBar(r.data[i],Forma2.loadSession().authResponse.signedRequest.permisos.alerts);
				...
				if(
					...
					|| Forma2.loadSession().authResponse.signedRequest.permisos && Forma2.loadSession().authResponse.signedRequest.permisos.alerts.delete == true
						...
						if(r.data[i],Forma2.loadSession().authResponse.signedRequest.permisos.alerts !== undefined && r.data[i],Forma2.loadSession().authResponse.signedRequest.permisos.alerts.create == true){
		...
		if(r.error === false){			
			$(".gantt").gantt({
				...
				onAddClick: function(dt, rowId) {
					if(Forma2.loadSession().authResponse.signedRequest.permisos.calendary.edit == true){
...
function verCalendario(target){
	...
	if(Forma2.loadSession().authResponse.signedRequest.permisos.calendary.edit == true){ $(".btn-edit-capa").show(); }
	else{ $(".btn-edit-capa").hide(); };
	if(Forma2.loadSession().authResponse.signedRequest.permisos.calendary.delete == true){ $(".btn-delete-capa").show(); }
...
function crearItemQuestionsPendingNavBarTop(element){
	if($("#question-response-navbar-id-"+element.id).length > 0){
		...
		Forma2.AccessToken()+"&id="+element.author.avatar;
		
		$("#menu-questions-response-top").append(crearItemNavBarTop({
			"id":"question-response-navbar-id-"+element.id,
			"picture":"api/v1.0/pictures.php?accesstoken="+Forma2.AccessToken()+"&id="+element.author.avatar,
...
function crearItemChatNavBarTop(element){
	if($("#chat-navbar-id-"+element.id).length > 0){
		...
		$("#menu-chat-top").append(crearItemNavBarTop({
			...
			"href":Forma2.options.site_url+"index.php?pageActive=messenger&read="+element.id+"&view=true",
			"description":
...
function cargarSideBar(){
	...
	if(Forma2.loadSession().authResponse.signedRequest.permisos && Forma2.loadSession().authResponse.signedRequest.permisos.ecards.view == true){
	...
	if(Forma2.loadSession().authResponse.signedRequest.permisos 
		&& Forma2.loadSession().authResponse.signedRequest.permisos.articles.view == true){
	...
	if(Forma2.loadSession().authResponse.signedRequest.permisos && Forma2.loadSession().authResponse.signedRequest.permisos.forum.view == true){
	...
	if(Forma2.loadSession().authResponse.signedRequest.permisos && Forma2.loadSession().authResponse.signedRequest.permisos.calendary.view == true){
	...
	if(
		Forma2.loadSession().authResponse.signedRequest.permisos && Forma2.loadSession().authResponse.signedRequest.permisos.alerts.history == true
		|| Forma2.loadSession().authResponse.signedRequest.permisos && Forma2.loadSession().authResponse.signedRequest.permisos.articles.history == true
		|| Forma2.loadSession().authResponse.signedRequest.permisos && Forma2.loadSession().authResponse.signedRequest.permisos.ecards.history == true
		|| Forma2.loadSession().authResponse.signedRequest.permisos && Forma2.loadSession().authResponse.signedRequest.permisos.quiz.history == true
	){
	...
	if(
		Forma2.loadSession().authResponse.signedRequest.permisos && Forma2.loadSession().authResponse.signedRequest.permisos.import.people == true
		|| Forma2.loadSession().authResponse.signedRequest.permisos && Forma2.loadSession().authResponse.signedRequest.permisos.import.indicators == true
	){
		...
		setTimeout(function(){
			$html = '<li>';
				...
				$html += '<ul class="collapse list-unstyled" id="importSubmenu">';
					if(Forma2.loadSession().authResponse.signedRequest.permisos && Forma2.loadSession().authResponse.signedRequest.permisos.import.people == true){
						$html += itemMenuSideBar({
							"name":"Personal",
							"icon":"fas fa-users",
							"href":"javascript: window.location.href = '"+Forma2.options.site_url+"index.php?pageActive=import-people'"
						});
					}
					
					if(Forma2.loadSession().authResponse.signedRequest.permisos && Forma2.loadSession().authResponse.signedRequest.permisos.import.indicators == true){
						$html += itemMenuSideBar({
							"name":"Indicadores",
							"icon":"fas fa-tasks",
	...
	if(Forma2.loadSession().authResponse.signedRequest.permisos && Forma2.loadSession().authResponse.signedRequest.permisos.export.quiz == true){
	...
	Forma2.app("POST", "quiz", 
	{
		...
		if(r.error === false){
			...
			if(r.quiz.querys.length > 0){
				window.location.href = Forma2.options.site_url+'index.php?pageActive=export-quiz&topic='+r.quiz.id;
			}else{
...
function cargarMenuHistorySideBar(){
	...
	$html += '<li>';
		...
		$html += '<ul class="collapse list-unstyled" id="historiesSubmenu">';
			...
			if(Forma2.loadSession().authResponse.signedRequest.permisos && Forma2.loadSession().authResponse.signedRequest.permisos.alerts.history == true){
				$html += itemMenuSideBar({
					"name":"Alertas",
					"icon":"far fa-bell",
					"href":"javascript: window.location.href = '"+Forma2.options.site_url+"index.php?pageActive=history-alerts'"
				});
			}
			if(Forma2.loadSession().authResponse.signedRequest.permisos && Forma2.loadSession().authResponse.signedRequest.permisos.articles.history == true){
				$html += itemMenuSideBar({
					"name":"Top Semanal",
					"icon":"glyphicon glyphicon-book",
					"href":"javascript: window.location.href = '"+Forma2.options.site_url+"index.php?pageActive=history-publish&type=articles'"
				});
			}
			if(Forma2.loadSession().authResponse.signedRequest.permisos && Forma2.loadSession().authResponse.signedRequest.permisos.ecards.history == true){
				$html += itemMenuSideBar({
					"name":"Info Banner",
					"icon":"glyphicon glyphicon-blackboard",
					"href":"javascript: window.location.href = '"+Forma2.options.site_url+"index.php?pageActive=history-publish&type=ecards'"
				});
			}
			if(Forma2.loadSession().authResponse.signedRequest.permisos && Forma2.loadSession().authResponse.signedRequest.permisos.ecards.history == true){
				$html += itemMenuSideBar({
					"name":"Evaluaciones",
					"icon":"fas fa-graduation-cap",
					"href":"javascript: window.location.href = '"+Forma2.options.site_url+"index.php?pageActive=history-quiz'"
				});
			};
		$html += '</ul>';
	$html += '</li>';
	...
	if(param.ref_id && param.topic && param.device_id && param.pageActive == 'view-vstep'){
		...
		Forma2.app("POST", "devices", 
		{
			...
			if(r.error === false){
				...
				$(".device-link").attr('href',Forma2.options.site_url+'index.php?pageActive=explore-category&type=devices&device_type='+r.data.device.type+'&device_id='+r.data.device.id);
				...
				HighlightOptionsTemp = {
					"cdn" : Forma2.options.api_url_large+'/pictures.php?accesstoken=',
					"api_url" : Forma2.options.api_url_large,
				...
				Forma2Manuals = {
					...
					Resources: { title_prefix: 'Forma2', title_suffix: ' - Forma2' },
...
$h += '<li class="list-group-item"><a href="'+Forma2.options.site_url+'index.php?pageActive=view-vstep&ref_id='+r.data.topics[i].items[a].id+'&device_id='+r.data.id+'&topic='+r.data.topics[i].id+'">'+r.data.topics[i].items[a].name+'</a></li>';
										}
									}
								$h += '</ul>';
							$h += '</div>';
...
function parseLiMenuSideBar(element,permisos){
	...
	if(element.name != undefined){
		...
		$html += '<li>';
			$html += '<a>';
				$html += '<span class="cursor-pointer" onclick="location.replace('+"'"+Forma2.options.site_url+'index.php?pageActive=explore-category&type='+element.type+'&of='+element.id+"&v=t'"+');">';
					$html += '<i class="'+element.icon+'"></i> ';
					$html += element.name+' ';
				$html += '</span>';
				if(permisos !== undefined && permisos.delete == true){
...
function cargarDatosPerfil(){
	...
	$(".forma2-micuenta-cargo-name").html($datos.cargo_name);
	$(".forma2-micuenta-rol-name").html($datos.rol_name);
	$('.forma2-micuenta-avatar-url').css('background-image', 'url(' + $datos.avatar_url + ')');
	$(".forma2-micuenta-piloto-name").html($datos.piloto_name);
}

function parsePublish($data){
	$link = Forma2.options.site_url+'index.php?pageActive=single&type='+$data.type+'&id_ref='+$data.id;
				...
				if(r.error === false && r.data.length > 0){
					...
					$.each(r.data,function(index, element){
						if($(".pinBoot #publish-id-"+element.id).length > 0){
							...
							$link = Forma2.options.site_url+'index.php?pageActive=single&type='+element.type+'&id_ref='+element.id;
...
function parseDevicesFeed($data){
	$link = Forma2.options.site_url+'index.php?pageActive=explore-category&type=devices&device_type='+$data.type+'&device_id='+$data.id;
...
function cargarAlertIndv(theId){
	...
	objeto_window_referencia = window.open(Forma2.options.site_url+'index.php?pageActive=notifications&view=single&id='+theId, "Visor de Notificaciones", configuracion_ventana);
}
			...
			if(result == true){
				Forma2.app("POST", "publicaciones", {
					...
					if(r.error === false){
						...
						location.replace(Forma2.options.site_url);
					}else{
	...
	if(param.type && param.id_ref){
		...
		Forma2.app("POST", "publicaciones", {
			...
			if(r.error === false){
				...
				$(".publish-page-view .photo-author").attr("src",'api/v1.0/pictures.php?accesstoken='+Forma2.AccessToken()+'&id='+r.data.author.avatar);
								  ...
								  reader.onload = function (e) {
									...
									Forma2.app("POST", "pictures", 
									{
										...
										if(r.error === false){
											...
											var url = Forma2.options.api_url_large+'/pictures.php?accesstoken='+Forma2.AccessToken()+"&id="+r.id;
...
function disableQuizAndEdit(id){
	Forma2.app("POST", "quiz", {
		...
		if(r.error === false){
			...
			setInterval(function(){ location.replace(Forma2.options.site_url+'/index.php?pageActive=create-quiz&type=quiz&draft='+id); },1000);
		}else{
...
function cargarVirtualStepsFelipheGomez(){ 
	(function($) {
		$(function() {
			var Forma2Manuals = window.Forma2Manuals || {};
			Forma2Manuals.HighlightHandler = $('.highlights').actions(window.Forma2Manuals.HighlightOptions);
			...
			$('.worldmanuals').on('click', '.blocks .pointer', function(e) {
				...
				window.Forma2Manuals.HighlightHandler.show($pointer.data(), true);
			...
			$('.feedback .question .comment a.send').on('click', function() {
				...
				var title = $("meta[property='og:title']").attr("content").replace(window.DeviceName + " - ", "").replace(" - Forma2", "");
			...
			window.Forma2Manuals = $.extend({}, window.Forma2Manuals, Forma2Manuals);
		});
		...
		if (this.files && this.files[0]) {
			...
			reader.onload = function (e) {
				Forma2.app("POST", "my", 
				{
					...
					if(r.error === false){
						imgId = r.id;
						$(".forma2-micuenta-link-profile").attr("src",Forma2.options.api_url_large+"/pictures.php?accesstoken="+Forma2.AccessToken()+"&id="+imgId);
